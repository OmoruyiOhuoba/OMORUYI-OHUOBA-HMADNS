...
export default function DoctorAppointmentTable({ bookedAppointments, deleteBookedSlots, doctorList, patientList, availableSlots, getAvailableSlots, getBookedSlots }) {
    ...
    const prescriptionFormSubmitted = async (event) => {
        ...
        let med = form.prescribedMed.value;
        console.log("medicine is" + med);
        console.log(reqObj)
        let response = await axios.post(`/api/omoruyiohuobahmadns/prescription/`,
            reqObj,
            {
                headers: {
        ...
        if (response.data.message == "success") {
            ...
            handlePrescriptionFormClose();
             ...
             let response2 = await axios.post(`/api/omoruyiohuobahmadns/drugnotification`,
             patientDetails);
             //alert(patientDetails); 
             console.log("izgoneeeee");
        }
    }
    ...
    const setFormProperties = async (appID) => {
        const response = await axios.get(`/api/omoruyiohuobahmadns/appointments/${appID}`, {
            headers: {
                authorization: `Bearer ${localStorage.getItem("token")}`
            }
        });
        ...
        // let response3 = await axios.get(`/api/omoruyiohuobahmadns/patients/${app.patientId}`)
        console.log("This is app oooo:"+ app.patientId);
        // console.log("this is the patient details" + response3)
        setDoctorId(app.doctorId)
        setPatientId(app.patientId)
        // setPatientDetails(app.patientId.userId);
    ...
    let rows = bookedAppointments.map((apt) => {
        ...
        patientDetails.firstName = apt.patientId.userId.firstName; 
        patientDetails.email = apt.patientId.userId.email;

        console.log("this is patients details ooo:" + patientDetails.firstName + "and" + patientDetails.email);
