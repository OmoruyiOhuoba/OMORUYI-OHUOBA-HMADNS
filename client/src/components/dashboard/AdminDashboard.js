...
export default function AdminDashboard() {
	...
	const getUserCountByRole = async (userType) => {
		const response = await axios.post(`/api/omoruyiohuobahmadns/count/users`,
			{
				'userType': userType
			},
			{
	...
	const getdoctors = async () => {
		const response = await axios.get("/api/omoruyiohuobahmadns/doctors");
		...
		<Box className={styles.dashboardBody} component="main" sx={{ flexGrow: 1, p: 3 }}>
			<div id={styles.welcomeBanner}>
				<div className='text-white'>
					...
					Online Hospital Management and Drug Notification System for Patients, Doctors and Admins. 
					<br/>
