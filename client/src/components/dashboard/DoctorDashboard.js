...
export default function DoctorDashboard() {
	...
	const getAppointmentCount = async () => {
		const response = await axios.get(`/api/omoruyiohuobahmadns/count/appointments`,
			{
				headers: {
	...
	const getPatientsTreatedCount = async () => {
		const response = await axios.get(`/api/omoruyiohuobahmadns/count/patients/treated`,
			{
				headers: {
					authorization: `Bearer ${localStorage.getItem("token")}`
				}
	...
	const getPrescription = async () => {
        ...
        let response = await axios.post(`/api/omoruyiohuobahmadns/prescriptions`,{},
            {
                headers: {
                    authorization: `Bearer ${localStorage.getItem("token")}`
                }
		...
		<Box className={styles.dashboardBody} component="main" sx={{ flexGrow: 1, p: 3 }}>
			<div id={styles.welcomeBanner}>
				<div className='text-white'>
					...
					Online Hospital Management and Drug Notification System for Patients, Doctors and Admins. 					<br/>
					This system is used to manage medical data, book appointments, assign prescriptions and send drug notifications.
